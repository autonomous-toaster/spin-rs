...
use spin_rs::{CheckerBuilder, CheckResult, LuaModel, SearchMode, StorageMode};
...
const ALL_MODELS: &[ModelDef] = &[
    ModelDef { name: "plan_5tasks_3ltls", source: PLAN_5TASKS_3LTLS, expected_errors: 0 },
    ModelDef { name: "plan_20tasks_10ltls", source: PLAN_20TASKS_10LTLS, expected_errors: 0 },
    ModelDef { name: "assertion_safety", source: ASSERTION_SAFETY, expected_errors: 0 },
    ModelDef { name: "multi_process", source: MULTI_PROCESS, expected_errors: 0 },
    ModelDef { name: "deadlock_circular", source: DEADLOCK_CIRCULAR, expected_errors: 1 },
    ModelDef { name: "ltl_violation", source: LTL_VIOLATION, expected_errors: 1 },
    ModelDef { name: "peterson_n2", source: PETERSON_N2, expected_errors: 0 },
    ModelDef { name: "peterson_n3", source: PETERSON_N3, expected_errors: 0 },
    ModelDef { name: "dining_n4", source: DINING_N4, expected_errors: 0 },
    ModelDef { name: "token_ring_n5", source: TOKEN_RING_N5, expected_errors: 0 },
    ModelDef { name: "state_explosion", source: STATE_EXPLOSION, expected_errors: 0 },
    ModelDef { name: "single_loop", source: SINGLE_LOOP, expected_errors: 0 },
];
...
fn phase1_correctness(spin_available: bool) -> (Vec<serde_json::Value>, String) {
    ...
    for model in ALL_MODELS {
        ...
        for &(config, search, por, tol) in &[
            ...
            let sp = if spin_available { run_spin(model.source, search == SearchMode::BreadthFirst, por) } else { None };
            let status = check_correctness(&sr, sp.as_ref(), model.expected_errors, tol);
            let icon = match &status { Ok(()) => "✓", Err(s) if s.starts_with("state") => "⚠", _ => "✗" };
            let detail = match &status { Ok(()) => String::new(), Err(e) => format!(": {e}") };
...
fn check_correctness(sr: &CheckResult, sp: Option<&SpinOutput>, expected: usize, tol: usize) -> Result<(), String> {
    if sr.errors != expected { return Err(format!("spin-rs errors {} != expected {}", sr.errors, expected)); }
    if let Some(sp) = sp {
        if sr.errors != sp.errors { return Err(format!("errors spin-rs={} spin={}", sr.errors, sp.errors)); }
        if tol == 0 && sr.states_explored != sp.states { return Err(format!("states spin-rs={} spin={}", sr.states_explored, sp.states)); }
        if tol > 0 {
            let s = sp.states.max(1);
            let d = if sr.states_explored > s { (sr.states_explored - s) * 100 / s } else { (s - sr.states_explored) * 100 / s };
            if d > tol { return Err(format!("states differ by {d}% (tol {tol}%)")); }
        }
    }
    Ok(())
}
...
fn phase2_local_perf() -> (Vec<serde_json::Value>, String) {
    ...
    println!("{:<25} {:>8} {:>8} {:>8} {:>8} {:>6} {:>10}", "Model", "Parse",
             "CG", "Boot", "Verify", "States", "T/s");
    println!("{}", "-".repeat(85));

    let models = ["plan_5tasks_3ltls", "single_loop", "state_explosion"];
    for model in ALL_MODELS.iter().filter(|m| models.contains(&m.name)) {
        let start = Instant::now(); let ast = spin_rs::parse(model.source).unwrap();
        let p = start.elapsed().as_micros();
        let start = Instant::now(); let _lua = spin_rs::generate_lua(&ast);
        let cg = start.elapsed().as_micros();
        let start = Instant::now(); let r = spin_rs::verify(model.source).unwrap();
        let v = start.elapsed().as_micros();
        let start = Instant::now(); let _lm = spin_rs::create_model(model.source).unwrap();
        let b = start.elapsed().as_micros();
        let tps = if v > 0 { r.transitions as f64 / (v as f64 / 1_000_000.0) } else { 0.0 };
        println!("{:<25} {:>8} {:>8} {:>8} {:>8} {:>6} {:>10.0}",
...
fn phase3_global(spin_available: bool) -> (Vec<serde_json::Value>, String) {
    ...
    for model in ALL_MODELS {
        ...
        let mut t = Vec::new();
        for _ in 0..5 { let s = Instant::now(); let _ = spin_rs::verify(model.source); t.push(s.elapsed().as_secs_f64()); }
        ...
        let sp_med = if spin_available {
            ...
            let mut t2 = Vec::new();
            for _ in 0..5 { let s = Instant::now(); spin_run_compiled(); t2.push(s.elapsed().as_secs_f64()); }
            t2.sort_by(|a, b| a.partial_cmp(b).unwrap());
            Some(t2[2])
        } else { None };

        let spd = match sp_med { Some(s) if s > 0.0 => format!("{:.1}x", s / sr_med), _ => "N/A".into() };
        let sts = if sr_med > 0.0 { format!("{:.0}", states as f64 / sr_med) } else { "N/A".into() };
        let sps = match sp_med { Some(s) => format!("{s:.4}"), None => "N/A".into() };

        println!("{:<25} {:>10.4} {:>10} {:>10} {:>10} {:>10}",
...
#[derive(Debug, Clone)]
struct SpinOutput { states: usize, transitions: usize, errors: usize, depth: usize }
...
fn run_spin(source: &str, bfs: bool, por: bool) -> Option<SpinOutput> {
    ...
    Command::new("spin").arg("-a").arg(tmp.join("model.pml")).current_dir(&tmp)
        .stdout(Stdio::null()).stderr(Stdio::null()).output().ok().filter(|o| o.status.success())?;
    let mut g = Command::new("gcc"); g.args(["-O2", "-o", "pan", "pan.c"]).current_dir(&tmp)
        .stdout(Stdio::null()).stderr(Stdio::null());
    if !por { g.arg("-DNOREDUCE"); }
    g.output().ok().filter(|o| o.status.success())?;
    let mut p = Command::new("./pan"); p.current_dir(&tmp).stdout(Stdio::piped()).stderr(Stdio::null());
    if bfs { p.arg("-f"); } else { p.arg("-n"); }
    let o = p.output().ok()?;
...
fn spin_compile(source: &str) -> Result<(), String> {
    ...
    Command::new("spin").arg("-a").arg(tmp.join("model.pml")).current_dir(&tmp)
        .stdout(Stdio::null()).stderr(Stdio::null()).output().map_err(|e| e.to_string())
        .and_then(|o| if o.status.success() { Ok(()) } else { Err("spin -a failed".into()) })?;
    Command::new("gcc").args(["-O2", "-o", "pan", "pan.c"]).current_dir(&tmp)
        .stdout(Stdio::null()).stderr(Stdio::null()).output().map_err(|e| e.to_string())
        .and_then(|o| if o.status.success() { Ok(()) } else { Err("gcc failed".into()) })
}

fn spin_run_compiled() -> Option<SpinOutput> {
    let tmp = std::env::temp_dir().join("spin_bench");
    let o = Command::new("./pan").arg("-n").current_dir(&tmp)
        .stdout(Stdio::piped()).stderr(Stdio::null()).output().ok()?;
    parse_spin(&String::from_utf8_lossy(&o.stdout))
}

fn parse_spin(stdout: &str) -> Option<SpinOutput> {
    let mut states = 0; let mut transitions = 0; let mut errors = 0;
    for line in stdout.lines() {
        let t = line.trim();
        if t.starts_with("State-vector") && t.contains("errors:")
            && let Some(e) = t.split("errors:").nth(1).and_then(|s| s.split_whitespace().next()).and_then(|s| s.parse().ok())
        { errors = e; }
        if t.contains("states, stored") && let Some(n) = t.split_whitespace().next().and_then(|s| s.parse().ok()) { states = n; }
        if t.contains("transitions") && !t.starts_with("#") && let Some(n) = t.split_whitespace().next().and_then(|s| s.parse().ok()) { transitions = n; }
    }
    Some(SpinOutput { states, transitions, errors, depth: 0 })
}
...
fn run_spin_rs(source: &str, storage: StorageMode, search: SearchMode, por: bool) -> CheckResult {
    CheckerBuilder::new()
        ...
        .storage_mode(storage).search_mode(search).por_enabled(por)
        .max_states(1_000_000).max_depth(100_000).build().check()
}
