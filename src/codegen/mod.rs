...
impl LuaGenerator {
    ...
    fn emit_assignment_effect(&mut self, target: &str, value: &Expression) {
        let expr_str = self.expr_to_lua(value);
        self.emit(&format!("s.{} = {}", target, expr_str));
    }

    fn emit_skip_effect(&mut self) {
        self.emit("-- skip");
    }

    fn emit_break_effect(&mut self) {
    ...
    fn emit_assert_effect(&mut self, expr: &Expression) {
        let e = self.expr_to_lua(expr);
        self.emit(&format!("_spin_assert({}, 'assertion failed')", e));
    }

    fn emit_goto_effect(&mut self) {
        self.emit("-- goto");
    }

    fn emit_expr_effect(&mut self) {
        self.emit("-- expr");
    }

    fn emit_printf_effect(&mut self, fmt: &str, args: &[Expression]) {
        let args_str: Vec<String> = args.iter().map(|a| self.expr_to_lua(a)).collect();
        let args_concat = args_str.join(", ");
        self.emit(&format!("_spin_printf('{}', {})", fmt, args_concat));
    }

    fn emit_run_effect(&mut self, name: &str, args: &[Expression]) {
        let args_str: Vec<String> = args.iter().map(|a| self.expr_to_lua(a)).collect();
        let args_concat = args_str.join(", ");
        self.emit(&format!("run({}, {})", name, args_concat));
    }
    ...
    fn emit_atomic_effect(&mut self, body: &[Stmt]) {
        for s in body {
            self.emit_effect_for_stmt(s);
        }
    }

    fn emit_unless_effect(&mut self, body: &[Stmt]) {
        for s in body {
            self.emit_effect_for_stmt(s);
        }
    }

    fn emit_decl_effect(&mut self) {
        self.emit("-- decl/label in d_step");
    }

    fn emit_for_effect(&mut self) {
        self.emit("-- for loop in d_step (deferred)");
    }

    fn emit_if_effect(&mut self, guards: &[Guard]) {
    ...
    fn emit_do_effect(&mut self, guards: &[Guard]) {
        if let Some(guard) = guards.first() {
            let cond_str = match &guard.condition {
                Some(e) => self.expr_to_lua(e),
                None => "true".to_string(),
